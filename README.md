...
Ultimately, the file sizes will be smaller because there will be multiple events for the same repo, and only the most recent entry per month will be kept.

#### Accessors
pullRequestEvent.repo.id
pullRequestEvent.payload.pull_request.head.repo.language
pullRequestEvent.payload.pull_request.head.repo.stargazers_count
pullRequestEvent.payload.pull_request.head.repo.watchers_count
